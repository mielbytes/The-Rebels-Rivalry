...
function officer:init(x, y)
    ...
    --officer Animations--
    self.img = love.graphics.newImage("assets/images/officer.png")
...
function officer:detect(dt)
    moving_towards_player = false
    local query = world:queryCircleArea(self.x, self.y,256,  {"Player"})
...
function officer:move_toward_player(query)
    attack_delay = attack_delay - 1 * love.timer.getDelta()
    for i, v in pairs(query) do
        local player = v:getObject()
...
function officer:take_damage(amount)
